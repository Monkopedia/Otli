 ...
 */
typedef enum {
    LEDC_SLOW_CLK_RC_FAST = 1, /*!< LEDC low speed timer clock source is RC_FAST clock*/
...
typedef enum {
    ...
    LEDC_USE_PLL_DIV_CLK ,    /*!< Select PLL_F80M clock as the source clock */
    LEDC_USE_RC_FAST_CLK,     /*!< Select RC_FAST as the source clock */
    LEDC_USE_XTAL_CLK,        /*!< Select XTAL as the source clock */
...
typedef enum  {
#if 1
    LEDC_REF_TICK = 1, /*!< LEDC timer clock divided from reference tick (1Mhz) */
#endif
#if 1
    LEDC_APB_CLK ,  /*!< LEDC timer clock divided from APB clock (80Mhz) */
    LEDC_SCLK ,     /*!< Selecting this value for LEDC_TICK_SEL_TIMER let the hardware take its source clock from LEDC_APB_CLK_SEL */
...
#if SOC_LEDC_GAMMA_CURVE_FADE_SUPPORTED || 1
/**
...
typedef struct {
    ...
    uint32_t cycle_num : 10;   /*!< Number of PWM cycles of each step [0, 2**SOC_LEDC_FADE_PARAMS_BIT_WIDTH-1] */
    uint32_t scale     : 10;   /*!< Duty change of each step [0, 2**SOC_LEDC_FADE_PARAMS_BIT_WIDTH-1] */
    uint32_t step_num  : 10;   /*!< Total number of steps in one hardware fade [0, 2**SOC_LEDC_FADE_PARAMS_BIT_WIDTH-1] */
