...
internal fun checkStepIsPositive(isPositive: Boolean, step: Number) {
    if (!isPositive) throw Throwable("Step must be positive, was: $step.")
}
...
public fun IntProgression.first(): Int {
    if (isEmpty())
        throw Throwable("Progression $this is empty.")
    return this.first
}
...
public fun <T : Comparable<T>> T.coerceIn(minimumValue: T?, maximumValue: T?): T {
    if (minimumValue !== null && maximumValue !== null) {
        if (minimumValue > maximumValue) throw Throwable("Cannot coerce value to an empty range: maximum $maximumValue is less than minimum $minimumValue.")
...
public fun <T : Comparable<T>> T.coerceIn(range: ClosedFloatingPointRange<T>): T {
    if (range.isEmpty()) throw Throwable("Cannot coerce value to an empty range: $range.")
    return when {
